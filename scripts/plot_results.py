...
expfname = "gridSearchYahoo.txt"

def write_line(f, line):
    f.write(line)
    f.write("\n")

def plot_means_in_time(experiment_name, experiment_num, axarr):
	styles = itertools.cycle((':','-.','--','-'))
	#styles = itertools.cycle(('-'))

	#file = open("exp_config/experiments_config.txt", "r")
	file = open("exp_config/"+expfname,'r')
...
def plot_regret(cur_experiment, experiment_num, axarr):
	file = open("exp_config/"+expfname, "r")
	colors = itertools.cycle(['blue','green','red','cyan','magenta','y','black','darkorange'])
	markers = itertools.cycle(['p','o','>','v','D','s','*'])
	styles = itertools.cycle(('-','-.','--','-'))
	#colors = itertools.cycle(matplotlib.colors.mcolors.TABLEAU_COLORS)
	#colors = itertools.cycle(list(plt.cm.tab10.colors))
	line_splitted = file.readline().split(" ")
	while(line_splitted[0] != cur_experiment):
		line_splitted = file.readline().split(" ")
	num_algs = int(line_splitted[3])
	
	axarr[1].set_xlabel("$n$", fontsize=big_font_size)
	axarr[1].set_ylabel(r"$\bar R_{N}$", fontsize=big_font_size)
	axarr[1].set_title("regret of algorithms("+cur_experiment+")")
	ttl = axarr[1].title
	...
	axarr[1].set_ylim(ymin=ymin_regret, ymax=lowest_final_regret+1500)
	...
	for i, (alg_name, ll) in enumerate(yss.items()):
		...
		# plot line
		p = axarr[1].plot(xs, means_and_stddevs[i][0], linestyle=next(styles), color=next(colors), marker=next(markers), markevery=200, linewidth=4, markersize=15, label=alg_name)
		...
		#patches_3.append(mpatches.Patch(color=p[0].get_color(), label=alg_name, marker=p[0].get_marker()))

	final_info_file = open("results/" + experiment_num + ".txt", "w")
	...
	axarr[1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), fancybox=True, shadow=True, ncol=2)
	plt.rcParams.update({'font.size': big_font_size})

big_font_size = 26
small_font_size = 22
plt.rcParams.update({'font.size': big_font_size, 'mathtext.fontset':'stixsans'})
...
file = open("exp_config/"+expfname, "r")  #MODIFY HERE TO MODIFY EXPERIMENTS FILES
num_simulations = int(file.readline().split(" ")[1])

cur_exp = sys.argv[1] #Experiment number
name_exp = sys.argv[2] #Experiment Name
...
for experiment_name in dirs:
	if experiment_name == name_exp: ##
		f, axarr = plt.subplots(nrows=2, ncols=1, figsize=(15,20), dpi=80)
		plot_means_in_time(name_exp, cur_exp, axarr)
		plot_regret(name_exp, cur_exp, axarr)
		#plot_reward(experiment_name, axarr)
		#plot_alarms(experiment_name, axarr)
		plt.savefig("results/experiment_" + cur_exp + ".png", bbox_inches='tight')
		plt.close()

#concatenate_images()
os.system('echo "finito {0}"'.format(cur_exp))
