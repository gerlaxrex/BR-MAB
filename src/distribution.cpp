...
BetaDistribution::BetaDistribution(string name, int alpha, int beta, boost::mt19937& rng) : StationaryDistribution(name,rng){
	this->alpha = alpha;
	this->beta = beta;
	if(this->alpha < 1 || this->beta < 1){
		throw std::runtime_error("parameters for the beta distribution must be >= 1");
	}
}

double BetaDistribution::draw(int timestep){
	//double r = random_unit();
	boost::random::beta_distribution<> dist(this->alpha,this->beta);
	boost::variate_generator<boost::mt19937&, boost::random::beta_distribution<> > vg = boost::variate_generator<boost::mt19937&, boost::random::beta_distribution<> >(*(this->rng), dist);
	//double sample = boost::math::quantile(dist, r);
	double sample = (double)vg();
	return sample;
}

double BetaDistribution::get_mean(int timestep){
	return this->alpha/(this->alpha + this->beta);
}

string BetaDistribution::toFile(){
	return this->name + " SineBernoulli " + to_string(this->alpha) + " " + to_string(this->beta);
}

BetaDistribution& BetaDistribution::change_parameters(int newAlpha, int newBeta){
	this->alpha = newAlpha;
	this->beta = newBeta;
	return *this;
}