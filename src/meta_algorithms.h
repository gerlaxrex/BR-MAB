...
class Recurrent_CD_Algorithm: public MetaAlgorithm {
private:
	vector<CDT*> cdts_up, cdts_down, sr_cdts_up, sr_cdts_down;
	vector<int> sr_change_estimates;
	vector<vector<Concept>> concepts; //Data structure for saving the concepts for each arm.
	bool use_history;
	int max_history;
	int M;
	int timestep;
	...
	bool ucb_run;
	bool global;
	double alpha_level, d;

	vector<int> till_M;
	vector<int> last_concept;
	//vector<double> means;
	vector<vector<double>> collected_rewards, sr_collected_rewards;
	vector<bool> last_resets_up, equal_concept_found;
	set<int> to_be_reset;
		//int num_alarms_up, num_alarms_down;

	void reset_arm(int pulled_arm);
public:
	Recurrent_CD_Algorithm(string name, int num_of_arms, int M, double alpha, double d, string cdt_line, string sub_alg_line, bool use_history, int max_history, bool smart_resets, boost::mt19937& rng, MAB* mab);
...
class Recurrent_Global_CD_Algorithm: public MetaAlgorithm {
private:
	vector<CDT*> cdts_up, cdts_down, sr_cdts_up, sr_cdts_down;
	vector<int> sr_change_estimates;
	vector<int> arm_pulls_history; //We store the number of times
	vector<multiConcept> concepts; //Data structure for saving the general concept.
	bool use_history;
	int max_history;
	int M;
	int timestep;
	...
	vector<vector<double>> collected_rewards, sr_collected_rewards;
	vector<bool> last_resets_up;
	bool equal_concept_found;
	set<int> to_be_reset;

	void reset_arm(int pulled_arm);
public:
	Recurrent_Global_CD_Algorithm(string name, int num_of_arms, int M, double alpha, double d, string cdt_line, string sub_alg_line, bool use_history, int max_history, bool smart_resets, boost::mt19937& rng, MAB* mab);

	vector<unsigned> num_pulls_before(int t, int action, int& pos);

	vector<unsigned> num_pulls_after(int pos);
...
// M-UCB implementation

class Monitored_UCB : public MetaAlgorithm {
	private:
	...
	public:
		...
		Monitored_UCB(string name, int num_of_arms, unsigned windowSize, double thresh, string sub_alg_line, bool use_history, boost::mt19937& rng, MAB* mab);
