...
class BetaDistribution : public StationaryDistribution{
private:
	int alpha, beta;
public: 
	/**
	 * @param name     string, id of the distribution
	 * @param alpha    number of obtained successes (1s) of a binomial process
	 * @param beta     number of obtained failures (0s) of a binomial process
	 * @param rng      boost::mt19937&, rng used to generate data
	 */
	BetaDistribution(string name, int alpha, int beta, boost::mt19937& rng);

	/**
	 * @param  timestep int, timestep at which the reward is drawn
	 * @return a double which is the drawn parameter for the binomial
	 */
	double draw(int timestep) override;
	...
	/**
	 * @param newAlpha the new alpha parameter for the distribution
	 * @param newBeta the new beta parameter for the distribution
	 * @return the distribution itself
	 */
	BetaDistribution& change_parameters(int newAlpha, int newBeta);
};
