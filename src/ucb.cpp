...
BayesUCB::BayesUCB(string name, int num_of_arms) : UCB(name,num_of_arms){
	this->reset(-1);
	this->successes.assign(this->num_of_arms,1);
	this->failures.assign(this->num_of_arms,1);
}

void BayesUCB::reset(int action){
	this->MABAlgorithm::reset(action);
	if (action == -1) {
		this->successes.assign(this->num_of_arms, 1);
		this->failures.assign(this->num_of_arms, 1);
		...
		this->successes[action] = 1;
		this->failures[action] = 1;
	}
}


int BayesUCB::choose_action(){
	double bestQuantile = NEG_INF;
	int bestArm = -1;
	//For each arm compute the quantiles and select the max
	int tot_pulls = accumulate(this->num_of_pulls.begin(), this->num_of_pulls.end(), 0.) + 1; //Current time
	int const_term = pow(log(100000),3);
	for(int arm = 0; arm < this->num_of_arms; ++arm){
		boost::math::beta_distribution<> dist(this->successes[arm],this->failures[arm]);
		double prob = 1 - ((double)1/(tot_pulls*const_term));
		double q = quantile(dist, prob);
...
void BayesUCB::receive_reward(double reward, int pulled_arm){
	this->MABAlgorithm::receive_reward(reward,pulled_arm);
	if(reward > 0){
		this->successes[pulled_arm]++;
	}else{
		this->failures[pulled_arm]++;
	}
}
...
// RecSingle_CUSUM_UCB::RecSingle_CUSUM_UCB(string name, int num_of_arms, int M, double alpha, double eps, double h, int max_hist) : UCB(name,num_of_arms){
// 	this->reset(-1);
// 	this->concepts.assign(num_of_arms,{});
// 	this->M = M;
// 	this->max_history = max_hist;

// 	this->arm_counters.assign(num_of_arms,M);
// }

// void RecSingle_CUSUM_UCB::reset(int action){
// 	this->MABAlgorithm::reset(action);
// 	if (action == -1) {
// 		;
// 	}
...
// int RecSingle_CUSUM_UCB::choose_action(){
// 	double bestQuantile = NEG_INF;
// 	int bestArm = -1;
// 	//For each arm compute the quantiles and select the max
// 	int tot_pulls = accumulate(this->num_of_pulls.begin(), this->num_of_pulls.end(), 0.) + 1; //Current time
...
// void RecSingle_CUSUM_UCB::receive_reward(double reward, int pulled_arm){
// 	this->MABAlgorithm::receive_reward(reward,pulled_arm);
// 	this->means[pulled_arm] = (this->means[pulled_arm] * (this->num_of_pulls[pulled_arm] - 1) + reward) / this->num_of_pulls[pulled_arm];
// }